...
const int durationSec = 5; // Duration in seconds for the stimulation waveform
float frameBufferFraction = 0.5; // Buffer as a fraction of the frame period
...
void measureFramePeriod() {
  ...
  if (digitalRead(framePeriodPin) == HIGH) {
    ...
    if (measuringHigh) {
      ...
      // Generate opto pulse at the edges of each frame if stimulation is active
      if (stimulationActive) {
        // Set HIGH for Pulse width
        digitalWrite(optoPin, HIGH); 
        delayMicroseconds(pulseWidth/2);
        // Set LOW for buffer width 
        digitalWrite(optoPin, LOW);
        delayMicroseconds(bufferWidth*2);
        // Set HIGH for Pulse width
        digitalWrite(optoPin, HIGH); 
        delayMicroseconds(pulseWidth/2);
      }
