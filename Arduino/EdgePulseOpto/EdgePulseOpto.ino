  ...
  if (digitalRead(framePeriodPin) == HIGH) {
    ...
    if (measuringHigh) {
      ...
      if (stimulationActive) {
        ...
        unsigned long block1StartTime = micros();
        while (micros() - block1StartTime < stimWidth) {
        ...
        digitalWrite(optoPin, LOW); 
        delayMicroseconds(stimWidth*1.85);
        unsigned long block2StartTime = micros();
        while (micros() - block2StartTime < stimWidth) {
          digitalWrite(optoPin, HIGH);
          delayMicroseconds(5000);  // 5 ms ON pulse
          digitalWrite(optoPin, LOW);
          delayMicroseconds(45000);  // 45 ms OFF pulse
